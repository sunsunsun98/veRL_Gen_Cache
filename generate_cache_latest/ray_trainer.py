    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                cache_prompts = (
                    gen_batch_output.non_tensor_batch["raw_prompt"].copy()
                    if use_gen_cache and "raw_prompt" in gen_batch_output.non_tensor_batch
                    else None
                )
                ...
                with marked_timer("step", timing_raw):
                    if use_gen_cache:
                        ...
                        if reuse_pre_result is not None and reuse_pre_result.have_pre_rollouts:
                            gen_batch_output = reuse_pre_result.gen_batch
                    ...
                    cache_save_prompts = batch.non_tensor_batch.get("raw_prompt", cache_prompts)
                    if use_gen_cache and cache_save_prompts is not None and "old_log_probs" in batch.batch:
                        cache_enqueued = self.gc_mgr.cache.save_batch_async(
                            prompts=cache_save_prompts,
