...
def _normalize_prompt_key(prompt: Any) -> str:
    """Build a stable cache key from raw chat messages or decoded prompt text."""
    ...
    if isinstance(prompt, dict):
        try:
            ...
            return str(prompt)
    prompt = str(prompt)
    return prompt.removeprefix("user\n").removesuffix("\nassistant\n")
...
def align_prev_to_gen(
    *,
    ...
    responses = torch.full(
        ...
        pad_token_id,
        dtype=sample["response_ids"].dtype,
        device=sample["response_ids"].device,
    )
    prompts = prompt_src.to(device=sample["input_ids"].device, dtype=sample["input_ids"].dtype).clone()

    for idx, data in cached_tensors.items():
...
class GenCacheManager:
    ...
    def __init__(
        ...
        # 配置（按你原始配置取值，这里给默认语义）
        gen_cache_config = trainer_config.trainer.get("gen_cache", {})
        self.n_repeat: int = trainer_config.actor_rollout_ref.rollout.n
        self.save_path: str = gen_cache_config.get("save_path", "./his_data") or "./his_data"
        self.reuse_factor = gen_cache_config.get("reuse_factor", 0.0)
        self.reuse_max_len = gen_cache_config.get(
        ...
        # hash index  
        self.chunk_size = gen_cache_config.get("chunk_size", 1000)
        self.cache = HashFileCache(
            save_path=self.save_path,
            chunk_size=self.chunk_size,
            num_write_workers=gen_cache_config.get("num_write_workers", 1),
            num_query_workers=gen_cache_config.get("num_query_workers", 1),
            max_pending_batches=gen_cache_config.get("max_pending_batches", 8),
            drop_when_full=gen_cache_config.get("drop_when_full", True),
            overwrite=gen_cache_config.get("overwrite", True),
        )
        # 确保目录存在（如果需要落盘）
    ...
    def reuse_generation(self, 
        ...
        with marked_timer("pre_log_probs", timing_raw, color="blue"):
            ...
            with marked_timer("query_batch", timing_raw, color="yellow"):
            ...
            full_response_len = aligned_old_responses.shape[1]
            reuse_max_len = min(int(self.reuse_max_len), full_response_len)

            truncated_responses = aligned_old_responses[:, :reuse_max_len].clone()
            truncated_input_ids = torch.cat([prompt_ids, truncated_responses], dim=1)  
            truncated_resp_mask = (truncated_responses != self.pad_token_id).long()
            truncated_attention_mask = torch.cat([prompt_attn_mask, truncated_resp_mask], dim=1)
            ...
            if reuse_max_len < full_response_len:
                ...
                padding = torch.full(
                    (new_logp.shape[0], pad_len),
                    float('-inf'),
                    dtype=new_logp.dtype,
                    device=new_logp.device,
                )
                new_logp = torch.cat([new_logp, padding], dim=1)  # [B, full_response_len]

            print("=======enter compute_reuse_len_via_rejection======")
            with marked_timer("compute_reuse_len_via_rejection", timing_raw, color="olive"):
            ...
            idx_need_np = idx_need.cpu().numpy()
            for key, value in gen_batch.non_tensor_batch.items():
                need_dp.non_tensor_batch[key] = value[idx_need_np]
    ...
    def rebuild_generate_batch(
        ...
        with marked_timer("post-rollout", timing_raw, color="red"):
            if pre_result.idx_need.shape[0] > 0:
                ...
                continued_non_tensor_batch = gen_batch_output.non_tensor_batch
            else:
                gen_out_meta_info = {}
                continued_non_tensor_batch = {}
            ...
            gen_batch_output.non_tensor_batch = self._rebuild_non_tensor_batch(
                ...
                need_output_non_tensor=continued_non_tensor_batch,
                batch_size=B,
            )
    ...
    @staticmethod
    def _default_non_tensor_value(key: str) -> Any:
        """Default values for rows that were fully reused and skipped vLLM."""
        if key in {
    ...
    def _rebuild_non_tensor_batch(
        self,
        pre_result: ReuseRolloutResult,
        need_output_non_tensor: dict,
        batch_size: int,
    ) -> dict:
        """Merge original sample metadata with extra fields from continued requests."""
        non_tensor_batch = {
        ...
        for key in ("__num_turns__", "turn_scores", "tool_rewards"):
...
class HashFileCache:
    ...
    def __init__(
        self,
        save_path: str = "./his_data",
        chunk_size: int = 1000,
        num_write_workers: int = 1,
        num_query_workers: int = 1,
        ...
        self.num_query_workers = max(1, int(num_query_workers))
        self._query_executor = (
            ThreadPoolExecutor(max_workers=self.num_query_workers, thread_name_prefix="HashFileCacheQuery")
            if self.num_query_workers > 1
            else None
        )
        ...
        self._queue = queue.Queue(maxsize=max_pending_batches)
        self._threads = [
            threading.Thread(target=self._write_worker, name=f"HashFileCacheWriter-{idx}", daemon=True)
            for idx in range(max(1, int(num_write_workers)))
        ]
    ...
    def _query_one_prompt(self, prompts: List[str], prompt_idx: int, n_repeat: int) -> Dict[int, Dict[str, torch.Tensor]]:
        h = self._hash(_normalize_prompt_key(prompts[prompt_idx * n_repeat]))
        ...
        try:
            ...
            if not isinstance(data_list, list) or len(data_list) != n_repeat:
                return {}
            return {prompt_idx * n_repeat + j: data_list[j] for j in range(n_repeat)}
        except Exception:
            return {}
    ...
    def query_batch(self, prompts: List[str], n_repeat: int = 1) -> Dict[int, Dict[str, torch.Tensor]]:
        ...
        if self._query_executor is not None and num_unique_prompts > 1:
            for batch_items in self._query_executor.map(
                lambda i: self._query_one_prompt(prompts, i, n_repeat), range(num_unique_prompts)
            ):
                results.update(batch_items)
            return results
        
        for i in range(num_unique_prompts):
            h = self._hash(_normalize_prompt_key(prompts[i * n_repeat]))
            pt_path = self._get_path(h)
            # import pdb; pdb.set_trace()
    ...
    def _write_worker(self):
        while True:
            ...
            for i in range(num_unique_prompts):
                ...
                prompt_str = _normalize_prompt_key(prompts[i * n_repeat])
                grouped_data_list = []
                for j in range(n_repeat):
                    idx = i * n_repeat + j
                ...
                pt_path = self._get_path(h)
                # import pdb; pdb.set_trace()
                if not self.overwrite and os.path.exists(pt_path):
                    continue
                try:
                    self._atomic_save(grouped_data_list, pt_path)
                except Exception:
                    self.write_errors += 1
