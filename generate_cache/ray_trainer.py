...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        use_gen_cache = gen_cache_config.get("use_gen_cache", False)
        if use_gen_cache:
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                cache_prompts = (
                    gen_batch.non_tensor_batch["raw_prompt"].copy()
                    if use_gen_cache and "raw_prompt" in gen_batch.non_tensor_batch
                    else None
                )
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # SYB fix
                    if use_gen_cache and reuse_pre_result is not None and reuse_pre_result.have_pre_rollouts:
                    ...
                    if bypass_recomputing_logprobs:  # Use `rollout_log_probs`
                        ...
                        with marked_timer("old_log_prob", timing_raw, color="blue"):
                            ...
                            old_logp_now = old_log_prob.batch['old_log_probs'].to(dtype=torch.float32)
                    ...
                    if self.use_reference_policy:
                        ...
                        with marked_timer(str(Role.RefPolicy), timing_raw, color="olive"):
                            ...
                            ref_logp_now = ref_log_prob.batch["ref_log_prob"].to(dtype=torch.float32)
                    ...
                    if use_gen_cache and cache_prompts is not None and "old_log_probs" in batch.batch:
                        cache_enqueued = self.gc_mgr.cache.save_batch_async(
                            prompts=cache_prompts,
                            responses=batch.batch["responses"],
                            input_ids=batch.batch["prompts"],
                            old_logps=batch.batch["old_log_probs"],
                            n_repeat=self.config.actor_rollout_ref.rollout.n,
                        )
                        metrics["gen_cache/save_enqueued"] = float(cache_enqueued)
                        metrics["gen_cache/dropped_batches"] = self.gc_mgr.cache.dropped_batches
                        metrics["gen_cache/write_errors"] = self.gc_mgr.cache.write_errors
                ...
                if "old_log_probs" in batch.batch and "ref_log_prob" in batch.batch:
                    rollout_logp_now = batch.batch["ref_log_prob"].to(dtype=torch.float32)
                    old_logp_now = batch.batch["old_log_probs"].to(dtype=torch.float32)
                    response_mask = batch.batch["response_mask"].bool()
                    v1 = old_logp_now.masked_select(response_mask).detach()
                    v2 = rollout_logp_now.masked_select(response_mask).detach()

                    if len(v1) > 1:
                        result = torch.corrcoef(torch.stack([v1, v2], dim=0))
                        corr_coef = result[0][1].item()
                        metrics.update({
                            "training/pearson_coef": corr_coef,
                        })
